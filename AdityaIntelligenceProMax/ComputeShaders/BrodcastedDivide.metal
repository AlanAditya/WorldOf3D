...
template <typename T>
kernel void BrodcastedDivGPU_1Dgg(device T* result [[buffer(0)]], device const T* A [[buffer(1)]], device const T* B [[buffer(2)]], constant size_m& strideR [[buffer(3)]], constant size_m& strideA [[buffer(4)]], constant size_m& strideB [[buffer(5)]], uint gid [[thread_position_in_grid]]) {
...
template <typename T>
kernel void BrodcastedDivGPU_2Dgg(device T* result [[buffer(0)]], device const T* A [[buffer(1)]], device const T* B [[buffer(2)]], constant size_m* strideR [[buffer(3)]], constant size_m* strideA [[buffer(4)]], constant size_m* strideB [[buffer(5)]], uint2 gid [[thread_position_in_grid]]) {
    // gid is general and not based on result Buffer
    // Convert GID into respective axis index;
...
template <typename T>
kernel void BrodcastedDivGPU_3Dgg(device T* result [[buffer(0)]], device const T* A [[buffer(1)]], device const T* B [[buffer(2)]], constant size_m* strideR [[buffer(3)]], constant size_m* strideA [[buffer(4)]], constant size_m* strideB [[buffer(5)]], uint3 gid [[thread_position_in_grid]]) {
    // gid is general and not based on result Buffer
    // Convert GID into respective axis index;
...
#define INSTANTIATE_FROM_TYPE(src_idx, type) \
    instantiate_kernel("BrodcastedDivGPU_" #src_idx "_0", BrodcastedDivGPU_1Dgg, type); \
    instantiate_kernel("BrodcastedDivGPU_" #src_idx "_1", BrodcastedDivGPU_2Dgg, type); \
    instantiate_kernel("BrodcastedDivGPU_" #src_idx "_2", BrodcastedDivGPU_3Dgg, type); \
    instantiate_kernel("BrodcastedDivGPU_" #src_idx "_3", BrodcastedDivGPU_NDgg, type); \
...
// Not functional yet
// General Brodcasted Div ND version where gid is based on result buffer directly as result buffer is contiguous in memory
template <typename T>
kernel void BrodcastedDivGPU_NDg(device T* result [[buffer(0)]], device const T* A [[buffer(1)]], device const T* B [[buffer(2)]], constant size_m* strideR [[buffer(3)]], constant size_m* strideA [[buffer(4)]], constant size_m* strideB [[buffer(5)]], constant int& dims [[buffer(6)]], uint gid [[thread_position_in_grid]]) {
...
instantiate_kernel("BrodcastedDivGPU_0", BrodcastedDivGPU_NDg, float);
instantiate_kernel("BrodcastedDivGPU_1", BrodcastedDivGPU_NDg, half);
instantiate_kernel("BrodcastedDivGPU_2", BrodcastedDivGPU_NDg, uint8_t);
instantiate_kernel("BrodcastedDivGPU_3", BrodcastedDivGPU_NDg, int);
